...
function deps_graph(env::EnvCache, registries::Vector{Registry.RegistryInstance}, uuid_to_name::Dict{UUID,String},
    ...
    weak_all_compat = Dict{UUID,Dict{VersionNumber,Dict{UUID,VersionSpec}}}()
    ...
    while true
        ...
        for uuid in unseen
            ...
            weak_all_compat_u = get_or_make!(all_compat, uuid)
            ...
            if (julia_version != VERSION && is_unregistered_stdlib(uuid)) || uuid_is_stdlib
                ...
                weak_all_compat_u_vr = get_or_make!(weak_all_compat_u, v)
                for (_, other_uuid) in proj.weakdeps
                    push!(uuids, other_uuid)
                    weak_all_compat_u_vr[other_uuid] = VersionSpec()
                end
            else
                for reg in registries
                    ...
                    add_compat!(weak_all_compat_u, Registry.weak_compat_info(info))
                end
            end
        end
    end
    ...
    return Resolve.Graph(all_compat, weak_all_compat, uuid_to_name, reqs, fixed, false, julia_version),
           all_compat
end
        ...
        if weak
            ...
            if entry !== nothing
                for (weakname, weakuuid) in entry.weakdeps
                    ...
                    installed = get!(() -> Base.locate_package(pkgid) !== nothing, installed_cache, pkgid)
